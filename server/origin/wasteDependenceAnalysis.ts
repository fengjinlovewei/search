...
async function wasteDependenceAnalysis(this: Search) {
  // 所有的入口文件
  // changeFiles = [
  ...
  const tipName: string = '$use';
  // 被引用次数
  const cited: string = '$cited';
  //
  ...
  for (const changePathData of this.changeFiles) {
    ...
    //
    // 跳过已经过滤的
    if (indexPathData[tipName]) continue;

    // 创建一个队列
    let searchQueue: string[] = [...indexPathData.children];
    // 检查过的做一个标记，避免出现无限循环。
...
export { wasteDependenceAnalysis };
