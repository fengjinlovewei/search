...
exports.getPrimitive = exports.getMemberExpression = exports.getIdentifier = exports.getExportIdentifier = exports.getImportNamespace = void 0;
...
const none = Symbol();
// 当前文件的所有导出赋值到对象上
function getImportNamespace({ fullPath }, originValue) {
    const { all } = this.MAP[fullPath];
    const { body } = this.getAst(fullPath).program;
    let obj = {};
    for (const exportItem of all) {
        const { fullPath: newFullPath, ExportSpecifier, ExportNamespaceSpecifier, ExportAllDeclaration, } = exportItem;
        // 1.
        ...
        if (ExportNamespaceSpecifier) {
            obj[ExportNamespaceSpecifier] = this.getImportNamespace({ fullPath: newFullPath }, originValue);
        }
        // 3. export * from 'kkk';
        if (ExportAllDeclaration) {
            const newObj = this.getImportNamespace({ fullPath: newFullPath }, originValue);
        ...
        if (bodyItem.type === 'ExportNamedDeclaration' && !bodyItem.source) {
            ...
            if (bodyItem.declaration.type === 'VariableDeclaration') {
                for (const varItem of bodyItem.declaration.declarations) {
                    if (varItem.type === 'VariableDeclarator' &&
                ...
                const other = bodyItem.declaration;
                try {
                    if ((other === null || other === void 0 ? void 0 : other.id.type) === 'Identifier') {
                        const { v, t } = this.getPrimitive({
                            value: other === null || other === void 0 ? void 0 : other.body,
                            fullPath,
                        });
                        obj[other.id.name] = v;
                    }
            ...
            if (!bodyItem.declaration && bodyItem.specifiers.length) {
                for (const specifierItem of bodyItem.specifiers) {
                    if (specifierItem.type === 'ExportSpecifier' &&
...
exports.getImportNamespace = getImportNamespace;
// 获取导入变量对应其他文件的导出变量的原始值
function getExportIdentifier({ type, name, fullPath }, originValue) {
    // if (name === 'lll') {
    //   debugger;
    // }
    const { all } = this.MAP[fullPath];
    ...
    if (type === 'ImportSpecifier') {
        ...
        for (const exportItem of all) {
            const { fullPath: newFullPath, ExportSpecifier, ExportNamespaceSpecifier, } = exportItem;
            // export { thing1, thing2 } from './module-a.js';
            // export { n as n1, m as m1 } from './module-a3.js';
            for (const ExportSpecifierItem of ExportSpecifier) {
                const { currentName, originName } = ExportSpecifierItem;
            ...
            if (bodyItem.type === 'ExportNamedDeclaration' && !bodyItem.source) {
                ...
                if (bodyItem.declaration) {
                    if (bodyItem.declaration.type === 'VariableDeclaration') {
                        ...
                        const other = bodyItem.declaration;
                        try {
                            if ((other === null || other === void 0 ? void 0 : other.id.type) === 'Identifier' && (other === null || other === void 0 ? void 0 : other.id.name) === name) {
                                return this.getPrimitive({
                                    value: other === null || other === void 0 ? void 0 : other.body,
                                    fullPath,
                                });
...
// 追溯变量的原始值
function getIdentifier({ name, fullPath, originValue }) {
    //debugger;
    var _a;
    //   if (name === 'TABS') {
    //     debugger;
    ...
    for (const importItem of all) {
        const { fullPath, ImportDefaultSpecifier, ImportSpecifier, ImportNamespaceSpecifier, } = importItem;
    ...
    for (const bodyItem of ast.program.body) {
        ...
        if (bodyItem.type === 'ExportNamedDeclaration' &&
            ((_a = bodyItem === null || bodyItem === void 0 ? void 0 : bodyItem.declaration) === null || _a === void 0 ? void 0 : _a.type) === 'VariableDeclaration') {
 ...
 * @returns
 */
function getMemberExpression({ value, fullPath }) {
    let name = '';
    // a[b]
 ...
 * @returns
 */
function getPrimitive({ value, fullPath }) {
    var _a;
    // undefined 的字面量
    ...
    if (value.type === 'ArrayExpression') {
        ...
        for (const item of value.elements) {
            const { v, t } = this.getPrimitive({ value: item, fullPath });
            const vType = (0, util_1.getType)(v);
        ...
        for (const item of value.properties) {
            ...
            if (item.type === 'ObjectProperty') {
                if (item.key.type === 'Identifier') {
                    ...
                    const comment = (_a = item.leadingComments) === null || _a === void 0 ? void 0 : _a.map((item) => {
            ...
            if (item.type === 'SpreadElement') {
                ...
                if (isArray || isObject || isString) {
                    obj = Object.assign(Object.assign({}, obj), v);
                }
                else {
