...
exports.setIworkData = void 0;
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const util_1 = require("./util");
// 历史需求数据拼装成md的表格格式
function getIworkInfo(iworkList) {
    let str = `|\n|`;
    const keyList = [
        {
            name: '需求地址',
...
function setIworkData() {
    return __awaiter(this, void 0, void 0, function* () {
        const pb = new util_1.ProgressBar('分析进度', 40, this);
        const key = `[x]`;
        const keyEnd = `[d]`;
        const intoMasterReg = /Merge branch '(.*?)' into 'master'$/;
        const infoReg1 = /Resolve "(.*?)"\n\nCloses #(\d+?)\n\nSee merge request (.*?)!(\d+)/;
        const infoReg2 = /(.*?)\n\nSee merge request (.*?)!(\d+)/;
        const sliceBranch = /^(.+?)\/(.*)/;
        const isNumber = /^\d+$/;
        ...
        const branchTypeList = {
            ...
            bugfix: 'https://ee.58corp.com/w/bug/',
        ...
        /**
         "c366c90d4[x]要嵘赫[x]2021-10-29 02:46:02 +0000[x]Merge branch 'feature/wxnr-451' into 'master'[x]Resolve "feat:本地版全局弹窗新增"
          ...
          ...
         */
        const gitlogData = yield (0, util_1.RunCommand)(`git log --pretty=format:"${types}" ${this.gitLog}`);
        //
        /**
         [
          'c366c90d4',
        ...
        for (const kid of gitlogDatNew) {
            ...
            */
            ...
            if (regData) {
                let issuesText = '', closedId = '', mergeRequestId = '';
                if (infoData1) {
        ...
        iworkDataList.reduce((previousValue, currentValue) => {
            promiseAll.push((0, util_1.RunCommand)(`git diff --name-status ${previousValue.hash} ${currentValue.hash}`));
            return currentValue;
        });
         ...
         */
        const promiseAllData = yield Promise.all(promiseAll);
        ...
        */
        ...
        for (let i = 0; i < iworkDataListLen; i++) {
            ...
            try {
                ...
                //log(hash);
                yield (0, util_1.RunCommand)(`git checkout ${hash}`);
                // 重新编译
                yield this.runModules.creatFileTree.func();
                yield this.runModules.formatFiles.func();
                yield this.runModules.getEntryScope.func();
                // 每一个入口文件对每一个需求都要检测。
                this.scopeData.forEach((fileEntry) => {
                    ...
                    for (const changeFile of changeFiles) {
                        ...
                        // 先判断是否是需要排出的文件
                        const isExcPathsList = this.iworkExcPaths.filter((item) => changePath.indexOf(item) === 0);
                        // 如果当前文件是被排除的范围，那么直接跳过检查
                        if (isExcPathsList.length > 0)
                            continue;
        ...
        // 根据入口写入需求
        const writeMd = (entryPath, iworkList) => {
            if (iworkList.length === 0)
                return;
            // 获取入口对应的md文件路径
            ...
            const iworks = '\n\n## 历史需求\n\n' + iworksInfoStr;
            // 判断文件路径是否存在
            if (fs_1.default.existsSync(newFileEntry)) {
                const originMd = fs_1.default.readFileSync(newFileEntry, 'utf-8');
                let md = originMd;
                const mdData = (0, util_1.mdParse)(md);
                ...
                try {
                    if (originMd !== md) {
                        fs_1.default.writeFileSync(newFileEntry, md);
                        return true;
                    }
                }
                catch (e) {
                ...
                // 读出模板md
                const defauleMD = fs_1.default.readFileSync(path_1.default.resolve(__dirname, '../static/README.md'), 'utf-8');
                // 如果不存在md文件，直接写入模板 + 需求数据
                try {
                    fs_1.default.writeFileSync(newFileEntry, defauleMD + iworks);
                    return true;
                }
                catch (e) {
                    this.log(`\n\nerror: ${newFileEntry} 写入失败\n\n`);
        ...
        // 切换成原本的分支
        yield (0, util_1.RunCommand)(`git checkout ${this.branch}`);
...
exports.setIworkData = setIworkData;
